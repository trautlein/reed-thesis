...
sipmles answer to a problem.^[For a recent example of this, see how in 2008 Harry Reid (D) then the majority leader of the Senate, was able to remove the possibility of a nuclear waste repository from being created at Yucca Mountain.] Leaders in Congress have large sway over the leaders who they can choose to fill the many roles below them. They are also the "brand image" of their party, especially when their party does not control the presidency. Understanding the ideology of leaders is important for a Congressional scholar to better understand why leaders act the way the do.
...
```{r source R1, warning=F, message=F, echo=FALSE}
## REMOVE CHUNK LATER
...
violin_senate <- ggplot(senate_113, aes(factor(party), ideo)) + 
  geom_violin(adjust = 1) + geom_jitter(height = 0) + 
  coord_flip() + ylim(-.75, .75) + 
  xlab("Party") + ylab("Ideology (NOMINATE)") +
  ggtitle("Ideology in the Senate by Party for the 113th Congress")

violin_senate + theme_bw()
```
...
violin_house <- ggplot(house_113, aes(factor(party), ideo)) + 
  geom_violin(adjust = 1) + geom_jitter(aes(height = x2, color = x2)) + 
  coord_flip() + ylim(-.75, .75) + 
  xlab("Party") + ylab("Ideology (NOMINATE)") +
  ggtitle("Ideology in the House by Party for the 113th Congress")

ggsave("/Users/cus/thesis/trautlein_thesis/figures/PlotHouseMeans.pdf",
       plot = violin_house)

violin_house + theme_bw()
```

```{r echo=F}
label_house_113 <- house_113 %>%
  mutate(labelnames = ifelse(name %in% c("JONES") & statecode==47, name, ""))

label_violin_house <- ggplot(label_house_113, aes(factor(party), ideo)) + 
  geom_violin(adjust = 1) + geom_jitter(aes(height = x2, color = x2)) + 
  coord_flip() + ylim(-.75, .75) + 
  geom_text_repel(data = dplyr::filter(label_house_113, labelnames != ""), aes(label=labelnames)) + 
  xlab("Party") + ylab("Ideology (NOMINATE)") +
  ggtitle("Ideology in the House by Party for the 113th Congress")

label_violin_house

ggplot(data = senate_113, aes(x = ideo, y = x2)) + geom_point() + 
  geom_text_repel(aes(label=name))

party_colors <- c("R" = "red","D" = "blue")
#p + scale_colour_manual(values = cols)

ggplot(data = senate_113, aes(x = ideo, y = x2, color = party)) + geom_point() + 
  geom_text_repel(aes(label=name)) + 
...
label(path = "/Users/cus/thesis/trautlein_thesis/figures/PlotHouseMeans.pdf",
